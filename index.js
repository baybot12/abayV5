...
const apivhtear = 'beli biar work'
...
const client = new WAConnection()
   ...
   console.log(color('♥','white'),color('♥','red'),color('♥','white'),color('SQAN CODENYA!!','white'),color('YOU','red'),color('TUBE','white'),color('Panji Briant','yellow'))
})
...
// ANTI LINK GRUP
		        ...
		        setTimeout( () => {
			        client.groupRemove(from, [kic]).catch((e)=>{reply(`*𝐊𝐑𝐔𝐙 𝐁𝐎𝐓 HARUS JADI ADMINâ—*`)})
		        }, 3000)
		        setTimeout( () => {
			        client.updatePresence(from, Presence.composing)
			        reply("Hedsot....")
...
break
		...
		case 'resepmasakan':  
			...
			if (!isGroupAdmins) return reply(ind.admin())
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group 𝐊𝐑𝐔𝐙 𝐁𝐎𝐓 :\n\n\`\`\``
					no = 0
	...
	case 'hobby':
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname)) 
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh kontak Ownerku <•_•> ,Jangan Lupa Save Ya (*>*)',MessageType.text, { quoted: mek} )
					break    
           	case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
		...
		case 'next':
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  ...
                  reply(`*${pushname}*, Command *${prefix}${command}* Tidak Ada Di Dalam *${prefix}menu 𝐊𝐑𝐔𝐙 𝐁𝐎𝐓*`)
